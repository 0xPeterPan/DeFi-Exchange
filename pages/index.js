...
export default function Home() {
  ...
  // `reservedCD` keeps track of the Crypto Devil tokens Reserve balance in the Exchange contract
  ...
  // removeCD is the amount of `Crypto Devil` tokens that would be sent back to the user based on a certain number of `LP` tokens
  ...
  const [tokenToBeReceivedAfterSwap, settokenToBeReceivedAfterSwap] =
    useState(zero);
  // Keeps track of whether  `Eth` or `Crypto Devil` token is selected. If `Eth` is selected it means that the user
  // wants to swap some `Eth` for some `Crypto Devil` tokens and vice versa if `Eth` is not selected
  ...
  const getAmounts = async () => {
    try {
      ...
      // get the amount of `Crypto Devil` tokens held by the user
      const _cdBalance = await getCDTokensBalance(provider, address);
      // get the amount of `Crypto Devil` LP tokens held by the user
  ...
  /**
   * swapTokens: Swaps  `swapAmountWei` of Eth/Crypto Devil tokens with `tokenToBeReceivedAfterSwap` amount of Eth/Crypto Devil tokens.
   */
  ...
  /**
   * _getAmountOfTokensReceivedFromSwap:  Returns the number of Eth/Crypto Devil tokens that can be received
   * when the user swaps `_swapAmountWEI` amount of Eth/Crypto Devil tokens.
   */
   ...
   * then we calculate the crypto devil tokens he can add, given the Eth he wants to add by keeping the ratios
   * constant
   */
  ...
  const _getTokensAfterRemove = async (_removeLPTokens) => {
    try {
      ...
      const _ethBalance = await getEtherBalance(provider, null, true);
      // get the crypto devil token reserves from the contract
      const cryptoDevilTokenReserve = await getReserveOfCDTokens(provider);
      // call the getTokensAfterRemove from the utils folder
      const { _removeEther, _removeCD } = await getTokensAfterRemove(
        provider,
        removeLPTokenWei,
        _ethBalance,
        cryptoDevilTokenReserve
      );
  ...
  */
      ...
      return (
        <div>
          <div className={styles.description}>
            ...
            {utils.formatEther(cdBalance)} Crypto Devil Tokens
            <br />
            {utils.formatEther(ethBalance)} Ether
            <br />
            {utils.formatEther(lpBalance)} Crypto Devil LP tokens
          </div>
          <div>
            ...
            {utils.parseEther(reservedCD.toString()).eq(zero) ? (
              <div>
                ...
                <input
                  type="number"
                  placeholder="Amount of CryptoDevil tokens"
                  onChange={(e) =>
                    setAddCDTokens(
              ...
              <div>
                ...
                <div className={styles.inputDiv}>
                  {/* Convert the BigNumber to string using the formatEther function from ethers.js */}
                  {`You will need ${utils.formatEther(addCDTokens)} Crypto Devil
                  Tokens`}
                </div>
                <button className={styles.button1} onClick={_addLiquidity}>
                  Add
            ...
            <div>
              ...
              Devil Tokens and ${utils.formatEther(removeEther)} Eth`}
              </div>
      ...
      return (
        <div>
          ...
          >
            ...
            <option value="cryptoDevilToken">Crypto Devil Token</option>
          </select>
          <br />
          <div className={styles.inputDiv}>
            {/* Convert the BigNumber to string using the formatEther function from ethers.js */}
  ...
  return (
    <div>
      <Head>
        <title>Crypto Devils</title>
        <meta name="description" content="Whitelist-Dapp" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <div className={styles.main}>
        <div>
          <h1 className={styles.title}>Welcome to Crypto Devils Exchange!</h1>
          <div className={styles.description}>
            Exchange Ethereum &#60;&#62; Crypto Devil Tokens
          </div>
          <div>
            <button
        ...
        <div>
          <img className={styles.image} src="./cryptodevil.svg" alt="Deviloper Advocate image of an avocado with a red devil horns" />
        </div>
      </div>
